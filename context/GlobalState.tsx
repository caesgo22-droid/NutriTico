...
export const GlobalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [state, setState] = useState<AppState>(() => {
    const saved = localStorage.getItem('nutritico_pro_v2');
    return saved ? JSON.parse(saved) : defaultState;
  });

  useEffect(() => {
    localStorage.setItem('nutritico_pro_v2', JSON.stringify(state));
  }, [state]);
  ...
  const actions: AppActions = {
    ...
    resetApp: () => { setState(defaultState); localStorage.removeItem('nutritico_pro_v2'); }
  };
