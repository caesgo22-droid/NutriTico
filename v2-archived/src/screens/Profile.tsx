    ...
    return (
        <div className="flex-1 overflow-y-auto pb-32 bg-slate-50 no-scrollbar p-6 space-y-8">
            ...
            <section className="bg-white p-4 rounded-[3rem] shadow-sm border border-slate-100 space-y-1">
                {state.profile.pathologies.length > 0 && (
                    <div className="px-5 py-4 border-b border-slate-50">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Protocolos Activos</p>
                        <div className="flex flex-wrap gap-2">
                            {state.profile.pathologies.map(p => (
                                <span key={p} className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-black uppercase tracking-wider border border-emerald-100">
                                    {p === 'Hypertension' ? 'Hipertensión' : p === 'Diabetes' ? 'Diabetes' : p === 'MetabolicSyndrome' ? 'Sind. Metabólico' : 'Alto Rendimiento'}
                                </span>
                            ))}
                        </div>
                    </div>
